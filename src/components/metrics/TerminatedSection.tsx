...
const EXIT_CODE_MAP: Record<number, { name: string; suggestion: ReactNode }> = {
  ...
  126: {
    name: "命令调用错误",
    suggestion:
      "无法调用镜像中指定的命令，请确认命令路径是否正确及是否有执行权限。",
  },
  127: {
    name: "找不到文件或目录",
  ...
  139: {
    name: "分段错误 (SIGSEGV)",
    suggestion:
      "容器试图访问未分配给它的内存并被终止，请检查应用代码中的内存访问逻辑。",
  },
  143: {
    name: "优雅终止 (SIGTERM)",
    suggestion:
      "容器收到优雅终止信号后正常关闭，这通常是由外部请求引起的，可能是正常的调度行为。",
  },
  255: {
    name: "退出状态超出范围",
    suggestion:
...
export const TerminatedSection = ({
  state,
  index,
}: {
  ...
  const exitCodeInfo =
    state.exitCode !== undefined
      ? getExitCodeInfo(state.exitCode)
      : { name: "", suggestion: "" };

  return (
    <MetricSection
      title={`作业诊断信息 #${index + 1}`}
