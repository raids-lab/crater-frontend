              ...
              <span>
                {withLineNumbers && (
                  <span className="text-highlight-slate/50 mr-2.5 select-none">
                    {String(lineIndex + 1).padStart(String(tokens.length).length, ' ')}
                  </span>
                )}
                {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                {line.map((token: any, tokenIndex: number) => {
                  ...
                  const style = {
                    ...tokenStyle,
                    color: theme === 'light' ? 'var(--shiki-light)' : 'var(--shiki-dark)',
                  }
...
const ShikiCodeBlock = React.memo(
  ({ children, className, language, ...restProps }: CodeBlockProps) => {
    ...
    // 根据代码长度使用不同渲染策略
    const shouldUseLightRenderer = useMemo(() => code.length > 5000, [code.length])
