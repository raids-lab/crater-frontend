...
const refreshTokenFn = async (): Promise<string> => {
  ...
  const response = await instance.post<IResponse<IRefreshResponse>>('auth/refresh', data)
...
instance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (errorCode === ERROR_TOKEN_EXPIRED && originalRequest && !originalRequest._retry) {
